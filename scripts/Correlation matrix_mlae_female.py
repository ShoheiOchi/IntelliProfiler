...
data = pd.read_excel('/path/to/your/directory/Correlation matrix_male_female_4-16.xlsx')
...
def hist_stacked(ax, series, groupby, colors, **kwargs):
    ...
    for name, color in colors.items():
        ...
        ax.bar(bins[:-1], counts, width=np.diff(bins), bottom=bottom, color=color, edgecolor='black', align='edge', alpha=0.7)
        bottom += counts
...
def corrfunc(x, y, **kwargs):
    ...
    ax.annotate(f'{r:.2f}', xy=(.5, .5), xycoords=ax.transAxes, ha='center', va='center', fontsize=font_size, color='black')
    if p < 0.001:
